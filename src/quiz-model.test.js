import { adjustmentChoice } from "./adjustment-model.js";
...
const main = () => [..."EGJCHOD"].map((id) => a(id));
const topics = (answers) =>
  recommendLessons(answers).items.map(({ topic }) => topic);

test("every authored path terminates in 2–7 answers without repeating a question", () => {
  const paths = [],
    seen = new Set();
  function walk(answers) {
    ...
    assert.ok(answers.length < 7);
    seen.add(id);
    const q = questionById(id);
    const ids = q.adjustment
      ? q.graph.nodes.map(([id]) => id).filter((id) => !["A", "Y"].includes(id))
      : [];
    const choices = q.adjustment
      ? [
          ...Array.from({ length: 2 ** ids.length }, (_, mask) => ({
            id: adjustmentChoice(ids.filter((_, i) => mask & (1 << i))),
          })),
          { id: "impossible" },
        ]
      : q.choices;
    for (const choice of [...choices, unsureChoice])
      walk([...answers, a(id, choice.id)]);
  }
  walk([]);
  assert.ok(paths.length > 287);
  assert.equal(seen.size, 12);
  let advanced = 0;
  for (const answers of paths) {
    assert.ok(answers.length >= 2 && answers.length <= 7);
    ...
    if (result.kind === "advanced") {
      ...
      assert.deepEqual(
        ...
        main().map((a) => a.question),
      );
      assert.ok(answers.every(isCorrect));
...
test("adjustment alternatives retain their distinct gaps and allow clarification", () => {
  assert.equal(nextQuestion([a("E"), a("G", "set:C,M")]), "M");
  assert.equal(nextQuestion([a("E"), a("G", "none")]), "B");
  const mixed = [a("E"), a("G", "set:M"), a("M", "direct-only")];
  assert.deepEqual(topics(mixed), ["confounding", "mediator"]);
  const recovered = recommendLessons([a("E"), a("G", "set:M"), a("M")]);
  assert.equal(
  ...
  assert.equal(
    nextQuestion([a("E", "harm"), a("F"), a("B"), a("G", "none")]),
    null,
  );
});
...
test("the final answer counts even when the seven-question limit prevents a follow-up", () => {
  const overlapGap = [...main().slice(0, 5), a("O", "remove-severity")];
  ...
  assert.deepEqual(topics([...main().slice(0, 6), a("D", "ipw-aipw")]), [
    "double-robustness",
  ]);
  const recovery = [
    a("E", "harm"),
...
test("editing an answer discards the superseded path", () => {
  ...
  assert.deepEqual(answerQuestion(main(), 3, "adjust"), [
    ...main().slice(0, 3),
    a("C", "adjust"),
  ]);
  assert.throws(
    () => answerQuestion(main(), 7, "unsure"),
    /Invalid quiz answer/,
  );
...
test("the harder graph diagnoses the failed path without skipping other topics", () => {
  const prefix = [a("E"), a("G")];
  assert.equal(nextQuestion(prefix), "J");
  for (const choice of ["set:C", "set:L", "set:C,K", "none", "unsure"])
    assert.equal(nextQuestion([...prefix, a("J", choice)]), "C");
  assert.deepEqual(topics([...prefix, a("J", "set:P")]), ["confounding"]);
  assert.deepEqual(topics([...prefix, a("J", "set:C,K")]), ["collider"]);
  assert.notEqual(recommendLessons([...prefix, a("J")]).kind, "advanced");
});
