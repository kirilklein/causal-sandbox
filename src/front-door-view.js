...
export function frontDoorGraph({ world = "valid", selection = 0.6, result }) {
  ...
  return [false, true]
    .map((mobile) => {
      ...
      const node = (name, x, y, label, labelY, labelX = x) =>
        `<g><circle cx="${x}" cy="${y}" r="24" fill="var(--node-${name})" ${name === "U" ? 'stroke="var(--text-muted)" stroke-dasharray="4 4"' : ""}/><text x="${x}" y="${y + 6}" text-anchor="middle" class="fd-node-label">${name}</text><text x="${mobile && name !== "U" ? x + 37 : labelX}" y="${mobile && name !== "U" ? y - 15 : labelY}" text-anchor="${mobile && name !== "U" ? "start" : "middle"}" class="fd-graph-label">${label}</text></g>`;
