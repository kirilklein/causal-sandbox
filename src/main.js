...
<div class="controls"><div class="control-group"><div class="group-heading">TREATMENT <span>The effect you put into the world</span></div>${slider("direct", "A → Y", "How much treatment A directly changes outcome Y.", -1, 4, 0.1)}</div><div class="control-group"><div class="group-heading">OBSERVED CONFOUNDING <span class="c-color">C = C₁, C₂</span></div><div class="slider-pair">${slider("ca", "C → A", "How strongly observed C influences who receives treatment. Zero removes this path.", 0, 3, 0.1)}${slider("cy", "C → Y", "How strongly observed C changes outcome Y. Zero removes this path.", 0, 3, 0.1)}</div></div><div class="control-group"><div class="group-heading">HIDDEN CONFOUNDING <span class="u-color">U · never observed</span></div><div class="slider-pair">${slider("ua", "U → A", "How strongly hidden U influences who receives treatment. Zero removes this path.", 0, 3, 0.1)}${slider("uy", "U → Y", "How strongly hidden U changes outcome Y. Zero removes this path.", 0, 3, 0.1)}</div></div><details class="path-controls"><summary>Mediator pathway <span>A → M → Y</span></summary><div class="slider-pair">${slider("am", "A → M", "How strongly treatment A changes mediator M.", 0, 2, 0.1)}${slider("my", "M → Y", "How strongly mediator M changes outcome Y.", 0, 2, 0.1)}</div></details></div></section>
...
<div class="adjust-row"><span class="choice-explanation"><b>Adjust for</b><small>Variables the estimators actually use.</small></span>${["C", "M", "K"].map((k) => `<label class="adjust-option"><input type="checkbox" value="${k}"><b>${k}</b><span>${{ C: "Confounder", M: "Mediator", K: "Collider" }[k]}</span></label>`).join("")}</div><div class="model-controls" aria-label="Analyst models"><div class="model-intro"><span>ANALYST’S MODELS</span><small id="model-purpose">Choose how the analysis uses C₁ and C₂.</small><small id="model-availability">C groups two observed covariates: C₁ and C₂.</small></div><div class="model-pair">${["outcome", "treatment"].map((k) => `<label><span>${k === "outcome" ? "Outcome model" : "Treatment model (PS)"}</span><small id="${k}-purpose">${k === "outcome" ? "Predicts outcome Y under each treatment." : "Predicts each person’s chance of treatment."}</small><select id="${k}-model" aria-describedby="model-purpose model-availability ${k}-purpose ${k}-terms"><option value="main">C₁ and C₂ separately</option><option value="interaction">Include C₁ × C₂ interaction</option></select><small id="${k}-terms"></small></label>`).join("")}</div></div><div class="effect-chart" id="effects"></div><div class="estimate-key"><span><i class="dot close"></i>Within 0.15 of truth</span><span><i class="dot biased"></i>Further from truth</span><span>Fixed-sample point estimates, not intervals</span></div><div id="lesson" class="lesson" aria-live="polite"></div><div id="overlap" class="overlap"></div></section></div></div>
