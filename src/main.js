...
<div class="controls"><div class="control-group"><div class="group-heading">TREATMENT <span>The effect you put into the world</span></div>${slider("direct", "A → Y", "Direct effect", -1, 4, 0.1)}</div><div class="control-group"><div class="group-heading">${helpButton("confounder", "OBSERVED CONFOUNDING")}<span class="c-color">C = C₁, C₂</span></div><div class="slider-pair">${slider("ca", "C → A", "Treatment assignment", 0, 3, 0.1)}${slider("cy", "C → Y", "Outcome influence", 0, 3, 0.1)}</div></div><div class="control-group"><div class="group-heading">${helpButton("hidden", "HIDDEN CONFOUNDING")}<span class="u-color">U · never observed</span></div><div class="slider-pair">${slider("ua", "U → A", "Treatment assignment", 0, 3, 0.1)}${slider("uy", "U → Y", "Outcome influence", 0, 3, 0.1)}</div></div><details class="path-controls"><summary>Mediator pathway <span>A → M → Y</span></summary><div class="slider-pair">${slider("am", "A → M", "Mediator response", 0, 2, 0.1)}${slider("my", "M → Y", "Mediated influence", 0, 2, 0.1)}</div></details></div></section>
...
<div class="adjust-row"><span class="term-label">${helpButton("adjustment", "Adjust for")}</span>${["C", "M", "K"].map((k) => `<div class="adjust-choice"><label class="adjust-option"><input type="checkbox" value="${k}" aria-label="Adjust for ${k} (${{ C: "Confounder", M: "Mediator", K: "Collider" }[k]})"><b>${k}</b></label>${helpButton({ C: "confounder", M: "mediator", K: "collider" }[k])}</div>`).join("")}</div><div class="model-controls" aria-label="Analyst models"><div class="model-intro"><span>ANALYST’S MODELS</span><small id="model-availability">C groups two observed covariates: C₁ and C₂.</small></div><div class="model-pair">${["outcome", "treatment"].map((k) => `<div class="model-control"><div class="term-label"><label for="${k}-model">${k === "outcome" ? "Outcome model" : "Treatment model"}</label>${k === "treatment" ? helpButton("propensity", "Propensity score") : ""}</div><select id="${k}-model" aria-describedby="model-availability"><option value="main">Main effects</option><option value="interaction">Main effects + C₁ × C₂</option></select><small id="${k}-terms"></small></div>`).join("")}</div></div><div class="effect-chart" id="effects"></div><div class="estimate-key"><span><i class="dot close"></i>${helpButton("error", "Within 0.15 of truth")}</span><span><i class="dot biased"></i>Further from truth</span><span>Point estimates, not intervals</span></div><div id="lesson" class="lesson" aria-live="polite"></div><div id="overlap" class="overlap"><span id="overlap-warning"></span><span class="overlap-help"><span class="term-label">${helpButton("overlap")}</span><span class="term-label">${helpButton("ess")}</span></span></div></section></div></div>
...
<dialog id="about" aria-labelledby="about-title"><div class="dialog-heading"><span class="eyebrow">UNDER THE SURFACE</span><button id="close-about" aria-label="Close explanation">×</button></div><h2 id="about-title">A world with an answer key.</h2><details class="glossary"><summary>Glossary · causal terms in plain language</summary><dl>${Object.entries(
  ...
  .join(
    "",
  )}</dl></details><p>Each person has fixed, independent background noise. Treatment is assigned by a logistic probability; outcomes follow these structural equations.</p><pre>C₁, C₂ ~ independent Uniform(-√3, √3)
...
function update() {
  ...
  document.querySelector("#model-availability").textContent = hasC
    ? "C includes both C₁ and C₂. Choose how they combine."
    : state.visible.has("C")
      ? "Adjust for C to model C₁, C₂ and their interaction."
      : "C is hidden. Neither model can use C₁ or C₂.";
